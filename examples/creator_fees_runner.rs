...
async fn main() -> Result<()> {
    ...
    let min_claimable_sol: f64 = std::env::var("MIN_CLAIMABLE_SOL")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(0.5);
    let reserve_sol: f64 = std::env::var("RESERVE_SOL")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(0.01);
    ...
    if claimable_sol >= min_claimable_sol {
        ...
        println!("below threshold ({min_claimable_sol} SOL); skipping claim");
    }

    let admin_balance = rpc.get_balance(&admin.pubkey()).await?;
    let reserve = sol_to_lamports(reserve_sol);
    let fee_estimate: u64 = 5_000;
    let transfer = admin_balance.saturating_sub(reserve.saturating_add(fee_estimate));
    if transfer == 0 {
        println!("nothing to sweep (balance {admin_balance}, reserve {reserve})");
        return Ok(());
    }
